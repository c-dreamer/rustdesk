// Give physical mouse input priority over remote input, so the local user can keep
// working while a remote peer is connected, instead of the two cursors fighting.
// Adapted from https://github.com/rustdesk/rustdesk/pull/16147 (closed upstream only
// because rustdesk declined external PRs for this feature, not for a technical reason;
// the maintainer called the approach sound reference material for their own future fix).
...
/// Option key gating this whole feature. `enable-` prefix means `option2bool` defaults
/// it to on (matches AnyDesk-parity being the expected default); set to "N" to opt out.
pub(crate) const LOCAL_INPUT_PRIORITY_OPTION: &str = "enable-local-input-priority";
...
// ponytail: WH_MOUSE_LL is installed once on the desktop this thread starts on. If
// Windows switches the input desktop (UAC secure desktop, another isolated service
// desktop), physical events on the new desktop stop being observed here while
// `try_change_desktop()` in input_service.rs keeps following the switch for remote
// injection, so protection can silently lapse for the life of that desktop switch.
// Upgrade path: recreate the hook on a desktop-switch notification, or replace the
// low-level hook with a raw input device registration, which is desktop-independent.
...
mod tests {
    use super::*;
    use crate::common::input::{MOUSE_BUTTON_LEFT, MOUSE_BUTTON_RIGHT};
